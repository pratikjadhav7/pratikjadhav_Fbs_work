...
void addPlayer() {
  ...
  if (count >= capacity) {
    capacity = capacity * 2;
    Player *temp = (Player *)realloc(p, capacity * sizeof(Player));
  ...
  do {
    ...
    if (p[count].JersyNum <= 0) {
      printf("Jersey Number must be positive! Try again.\n");
    }
  } while (p[count].JersyNum <= 0);
  ...
  do {
    ...
    if (p[count].runs <= 0) {
      printf("Runs must be positive! Try again.\n");
    }
  ...
  do {
    ...
    if (p[count].wkts <= 0) {
      printf("Wickets must be positive! Try again.\n");
    }
  } while (p[count].wkts <= 0);

  do {
    printf("\nNumber Of Matches Played: ");
    scanf("%d", &p[count].matchesPlayed);

    if (p[count].matchesPlayed <= 0) {
      printf("It must be positive! Try again.\n");
    }
  } while (p[count].matchesPlayed <= 0);
...
void searchByName() {
  ...
  for (int i = 0; i < count; i++) {
    if (strcasecmp(p[i].name, name) == 0) {
      found = 1;
      printf("%s :- Jersey Num = %d | Total Runs = %d | Total Wkts = %d | "
             "Matches Played = %d",
             p[i].name, p[i].JersyNum, p[i].runs, p[i].wkts,
             p[i].matchesPlayed);
    }
  }
  if (found == 0)
...
void searchByJersey() {
  ...
  for (int i = 0; i < count; i++) {
    if (jNo == p[i].JersyNum) {
      found = 1;
      printf("%d :- Player = %s | Total Runs = %d | Total Wkts = %d | Matches "
             "Played = %d",
             p[i].JersyNum, p[i].name, p[i].runs, p[i].wkts,
             p[i].matchesPlayed);
    }
  }
  if (found == 0)
...
void displayAll() {
  printf("\n-------All Players Details -------\n");
  for (int i = 0; i < count; i++) {
    printf("\nPLayer = %s || Jersey Number = %d || Total Runs = %d  || Total "
           "Wkts = %d  || Matches Played = %d\n",
